...
def get_commits(target_dir):
    try:
        ...
        print(f"Error getting commits in {target_dir}: {e}", file=sys.stderr)
        return []
...
def get_commit_files(target_dir, commit_hash):
    try:
        ...
        print(f"Error getting files for commit {commit_hash}: {e}", file=sys.stderr)
        return []
...
def get_file_content(target_dir, commit_hash, filepath):
    try:
        ...
        print(f"Error getting file content for {filepath} in commit {commit_hash}: {e}", file=sys.stderr)
    return None
