...
- [x] **Infrastructure Setup**:
    - [x] Create the `github-secret-report` repository on GitHub.
    - [x] Configure repository secrets (e.g., `REPORT_PUSH_TOKEN`, `SMTP_PASSWORD`, `NOTIFY_EMAIL`) for automated commits and notifications.
- [x] **Repository List Generation**:
    - [x] Create a script (e.g., `tools/curate_repos.py`) to generate `data/target_repos.json` containing 500 popular repositories.
    - [x] Create a script (`scripts/discover_repos.py`) to find 20 new repositories daily and append them as "Hasn't yet validated".
- [x] **Daily Workflow (GitHub Action in `github-secret-report`)**:
    - [x] Implement `.github/workflows/daily-secret-report.yml` in the new repo.
    - [x] Add a configurable setting `SCAN_COUNT` (default: 20) in the workflow.
    - [x] **Rotation Logic**: Implement a Python script to manage selection and update `data/scan_state.json` to ensure sequential coverage of the repositories.
    - [x] **E-mail Notification Step**: Use the `dawidd6/action-send-mail` action or similar best-practice approach. For advanced integrations, explore the GitHub REST API for creating issues/comments that trigger notifications, or use a dedicated e-mail service (SendGrid/Mailgun).
- [x] **Report Publishing**:
    - [x] Implement logic to format scan results into readable cumulative reports.
    - [x] Extract deep scan metrics (Files scanned, Remote Branches, Repository Size, Scan Duration).
    - [x] Automate the commit and push of report files to the target repository.

## 4. Testing & Verification (Mandatory)
### 4.1 Unit Testing
- [x] `test_repo_rotation`: Verify the logic for selecting the next $N$ repositories from the list of 500.
- [x] `test_report_formatting`: Ensure the generated report is correctly formatted and contains no raw secrets.

### 4.2 Acceptance Testing (BDD)
- [x] **Scenario**: Full Cycle (Run the scanning script locally on 3 mock repos and verify a report is generated and ready for push).
- [x] **Scenario**: Configurable Limit (Change $N=5$ and verify the system scans exactly 5 repositories).

### 4.3 Test Data
- [x] Use a small subset of the target list for testing the workflow integrity.
